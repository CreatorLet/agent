...
type ToolDef = {
  name: string;
  description: string;
  input_schema: { type: "object"; properties: Record<string, { type: string }>; required: string[]; additionalProperties: boolean };
};
...
5. Run builds, tests, or useful checks.
...
- Never start a long-running server yourself; the host starts it after the build.
- For Node web servers, listen on process.env.PORT with a fallback such as 3001 and bind normally.
- For static sites, keep index.html or produce a standard dist/build folder.
...
const tools: ToolDef[] = [
  { name: "list_files", description: "List workspace files/directories.", input_schema: { type: "object", properties: { directory: { type: "string" } }, required: [], additionalProperties: false } },
  { name: "read_file", description: "Read a UTF-8 text file.", input_schema: { type: "object", properties: { file_path: { type: "string" } }, required: ["file_path"], additionalProperties: false } },
  { name: "write_file", description: "Create or replace a UTF-8 text file. Parent directories are created automatically.", input_schema: { type: "object", properties: { file_path: { type: "string" }, content: { type: "string" } }, required: ["file_path", "content"], additionalProperties: false } },
  { name: "edit_file", description: "Replace one exact text fragment in a file. old_text must occur exactly once.", input_schema: { type: "object", properties: { file_path: { type: "string" }, old_text: { type: "string" }, new_text: { type: "string" } }, required: ["file_path", "old_text", "new_text"], additionalProperties: false } },
  { name: "delete_file", description: "Delete a file or empty directory inside the workspace.", input_schema: { type: "object", properties: { file_path: { type: "string" } }, required: ["file_path"], additionalProperties: false } },
  { name: "search_files", description: "Search text recursively in workspace text files.", input_schema: { type: "object", properties: { query: { type: "string" } }, required: ["query"], additionalProperties: false } },
  { name: "run_command", description: "Run a shell command for installs, builds, tests, or debugging. Do not use it for long-running servers.", input_schema: { type: "object", properties: { command: { type: "string" } }, required: ["command"], additionalProperties: false } },
];
...
export async function ensureWorkspace() { await fs.mkdir(ROOT, { recursive: true }); }
export async function cleanWorkspace() { await fs.rm(ROOT, { recursive: true, force: true }); await fs.mkdir(ROOT, { recursive: true }); }
...
export async function runAgent(provider: Provider, userPrompt: string, emit: (event: AgentEvent) => void) {
  ...
  if (provider === "groq") {
    ...
    for (let step = 1; step <= MAX_STEPS; step++) {
      const response = await client.chat.completions.create({ model: process.env.GROQ_MODEL || "openai/gpt-oss-120b", messages: [{ role: "system", content: SYSTEM }, ...messages], tools: openAiTools, tool_choice: "auto", temperature: 0 });
      ...
      for (const call of calls) {
        ...
        let result: string;
        try { result = await execute(name, normalizeArgs(call.function?.arguments)); }
  ...
  const messages: any[] = [{ role: "user", content: userPrompt }];
  const anthropicTools: any[] = tools.map(t => ({ name: t.name, description: t.description, input_schema: t.input_schema }));
  for (let step = 1; step <= MAX_STEPS; step++) {
    const response: any = await client.messages.create({ model: process.env.ANTHROPIC_MODEL || "claude-sonnet-4-20250514", max_tokens: 12_000, system: SYSTEM, messages, tools: anthropicTools });
    const toolResults: any[] = [];
    for (const block of response.content ?? []) {
      if (block.type === "text") emit({ type: "text", message: String(block.text) });
      if (block.type === "tool_use") {
        emit({ type: "tool", message: `Running ${block.name}` });
        let result: string;
        try { result = await execute(String(block.name), record(block.input) ? block.input : {}); }
        catch (error) { result = `ERROR: ${error instanceof Error ? error.message : String(error)}`; }
