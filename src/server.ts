...
async function startPreview() {
  ...
  if (await exists(dist) && await exists(path.join(dist, "index.html"))) {
    previewKind = "static"; previewRoot = dist; return;
  }
  if (await exists(build) && await exists(path.join(build, "index.html"))) {
    previewKind = "static"; previewRoot = build; return;
  }
  if (await exists(index)) {
    previewKind = "static"; previewRoot = ROOT; return;
  }

  const pkg = await readPackage();
  const startScript = pkg?.scripts?.start;
  if (!startScript) throw new Error("The agent finished, but no index.html/dist/build or npm start script was found for preview.");
  ...
  previewProcess.on("exit", code => {
    if (code && status === "ready") push({ type: "error", message: `Preview process exited with code ${code}.` });
    previewProcess = null;
  });
...
async function proxyPreview(req: express.Request, res: express.Response) {
  ...
  if (previewKind === "static") {
    ...
    try {
      ...
      if (stat.isFile()) return res.sendFile(safe);
    } catch { /* route below */ }
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Agent prototype listening on ${PORT}`);
});
